...
public enum BuiltInTemperatureModifier {
    ...
    private interface Modifier {
        Tuple<TemperatureLevel, Integer> apply(Player player, TemperatureLevel currentTarget, int currentChangeDelay);
    }
    ...
    public static List<BuiltInTemperatureModifier> getTemperatureModifierOrder() {
        if (temperatureModifierOrderCache == null) {
            temperatureModifierOrderCache = ModConfig.temperature.temperatureModifierOrder.stream()
                    .map(s -> BuiltInTemperatureModifier.valueOf(s.toUpperCase()))
                    .toList();
        }
