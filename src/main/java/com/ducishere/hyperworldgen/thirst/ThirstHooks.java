...
public class ThirstHooks {
    ...
    public static void doFoodDataTick(FoodData data, ServerPlayer player){
        ...
        if(data.exhaustionLevel > 4.0F){
            ...
            if(data.saturationLevel > 0.0F) data.saturationLevel = Math.max(data.saturationLevel-1,0);
            else if(difficulty != Difficulty.PEACEFUL) data.foodLevel = Math.max(data.foodLevel-1,0);
        }
